<div align="center">

# 🌟 Hi, I'm Alin Sanhanat - UX/UI Design Technologist

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&pause=1000&color=6366F1&center=true&vCenter=true&width=435&lines=UX%2FUI+Designer+%26+Developer;AI-Powered+Design+Specialist;Framer+Motion+Animation+Expert;WebGL+%26+Three.js+Enthusiast;Sustainable+Design+Advocate" alt="Typing SVG" />

</div>

## 🚀 About Me
- 🎓 **Digital Technology @ KMUTT** (Graduating 2027) | Targeting **Canada Express Entry**
- 🎨 **Design Philosophy**: Human-Centered Design meets Cutting-Edge Technology
- 🧠 **Specialization**: AI-Assisted UX Research, Advanced Prototyping, Performance Optimization
- 🌍 **Mission**: Creating sustainable, accessible digital experiences for global impact
- 📧 **Let's Connect**: your.email@gmail.com | [LinkedIn](link) | [Portfolio](link)

## 🛠️ Tech Arsenal 2025

### 🎨 Design & Prototyping
![Figma](https://img.shields.io/badge/-Figma-F24E1E?style=for-the-badge&logo=figma&logoColor=white)
![Framer](https://img.shields.io/badge/-Framer-0055FF?style=for-the-badge&logo=framer&logoColor=white)
![Adobe XD](https://img.shields.io/badge/-Adobe%20XD-FF26BE?style=for-the-badge&logo=adobe-xd&logoColor=white)
![Principle](https://img.shields.io/badge/-Principle-25A3E3?style=for-the-badge&logo=principle&logoColor=white)

### 💻 Advanced Development
![React](https://img.shields.io/badge/-React-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![Next.js](https://img.shields.io/badge/-Next.js-000000?style=for-the-badge&logo=next.js&logoColor=white)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![Three.js](https://img.shields.io/badge/-Three.js-000000?style=for-the-badge&logo=three.js&logoColor=white)

### 🎬 Animation & Motion
![Framer Motion](https://img.shields.io/badge/-Framer%20Motion-0055FF?style=for-the-badge&logo=framer&logoColor=white)
![GSAP](https://img.shields.io/badge/-GSAP-88CE02?style=for-the-badge&logo=greensock&logoColor=white)
![Lottie](https://img.shields.io/badge/-Lottie-00DDB3?style=for-the-badge&logo=airbnb&logoColor=white)
![WebGL](https://img.shields.io/badge/-WebGL-990000?style=for-the-badge&logo=webgl&logoColor=white)

### 🧠 AI & Research Tools
![ChatGPT](https://img.shields.io/badge/-ChatGPT-74AA9C?style=for-the-badge&logo=openai&logoColor=white)
![Midjourney](https://img.shields.io/badge/-Midjourney-000000?style=for-the-badge&logo=midjourney&logoColor=white)
![Maze](https://img.shields.io/badge/-Maze-4A90FF?style=for-the-badge&logo=maze&logoColor=white)
![Hotjar](https://img.shields.io/badge/-Hotjar-FF3C00?style=for-the-badge&logo=hotjar&logoColor=white)

## 📊 GitHub Analytics
<div align="center">
<img height="180em" src="https://github-readme-stats.vercel.app/api?username=yourusername&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true"/>
<img height="180em" src="https://github-readme-streak-stats.herokuapp.com/?user=yourusername&theme=tokyonight"/>
</div>

## 🏆 Featured Projects

### 🌱 [Carbon Footprint Tracker - Gamified Sustainability](link)
**UX Research → AI-Powered Prototype → React Development**
- 🔬 **Research**: 100+ user interviews, behavioral analysis
- 🎮 **Gamification**: Point systems, challenges, leaderboards
- 🎨 **Advanced UI**: Morphism, micro-interactions, dark mode
- 📱 **Tech**: React + Framer Motion + Chart.js + WebGL
- 📈 **Impact**: 85% task completion, 15% energy reduction

### 🛒 [Next-Gen E-commerce Experience](link)
**AI-Driven Personalization + 3D Product Visualization**
- 🤖 **AI Features**: Smart recommendations, predictive search
- 🎯 **3D Showcase**: Three.js product models, AR preview
- ⚡ **Performance**: 98 Lighthouse score, <2s load time
- 🎬 **Animations**: GSAP scroll triggers, layout animations
- 📊 **Results**: 40% conversion increase, 60% bounce reduction

### 🎭 [Interactive Portfolio 2025](link)
**WebGL + AI + Spatial Design**
- 🎨 **Spatial UI**: Apple Vision Pro inspired design
- 🎬 **Motion Design**: Physics-based animations, particle systems
- 🧠 **AI Integration**: Dynamic content generation
- 🌐 **Tech Stack**: Next.js + Three.js + Framer Motion + GSAP
- ♿ **Accessibility**: WCAG 2.1 AAA, screen reader optimized

### 🏥 [HealthTech Dashboard - Data Visualization](link)
**Complex Data + Intuitive Interface**
- 📊 **Data Viz**: D3.js, interactive charts, real-time updates
- 🎨 **Design System**: Atomic design, 100+ components
- 🔧 **Usability**: 95% SUS score, 5-second rule compliance
- 🎯 **User Testing**: A/B testing, heat mapping, eye tracking

## 🎨 Design Process & Methodology
graph LR
A[🔍 Research] --> B[🎯 Define]
B --> C[💡 Ideate]
C --> D[🎨 Prototype]
D --> E[🧪 Test]
E --> F[🚀 Develop]
F --> G[📊 Iterate]


### 🔬 Advanced UX Research
- **AI-Powered Analytics**: Predictive user behavior modeling
- **Biometric Testing**: Eye tracking, EEG, emotion recognition
- **Cross-Cultural Research**: Global user persona development
- **Accessibility Studies**: Inclusive design for diverse abilities

### 🎨 Cutting-Edge Design Trends 2025
- **Spatial Design**: 3D interfaces, depth perception
- **AI-Generated Assets**: Procedural design elements
- **Morphism Evolution**: Glass, clay, and liquid morphism
- **Kinetic Typography**: Motion-based text interactions
- **Voice UI Integration**: Conversational design patterns

## 🎬 Animation Showcase

### Framer Motion Mastery
// Advanced Layout Animation with Physics
const spring = {
type: "spring",
stiffness: 700,
damping: 30
};

const containerVariants = {
hidden: { opacity: 0 },
visible: {
opacity: 1,
transition: {
delayChildren: 0.3,
staggerChildren: 0.2
}
}
};


## 🌐 Live Demos & Prototypes

### 🎮 Interactive Prototypes
- 🔗 **Figma Prototypes**: [High-fidelity Interactive Demos](link)
- 🎨 **Framer Sites**: [Live Animation Showcases](link)
- 🌐 **CodePen**: [Advanced CSS/JS Experiments](link)
- 📱 **Mobile Prototypes**: [Touch Gesture Innovations](link)

### 🧪 Usability Testing Results
| Project | Task Success | SUS Score | Time on Task | Error Rate |
|---------|-------------|-----------|--------------|------------|
| Carbon Tracker | 92% | 87 | 2.3min | 3% |
| E-commerce | 96% | 91 | 1.8min | 2% |
| Portfolio | 89% | 85 | 3.1min | 4% |

## 🏅 Certifications & Learning

### 🎓 2025 Certifications
- **Google UX Design Professional** ✅
- **Adobe Certified Expert - XD** ✅
- **Figma Advanced Certification** ✅
- **HCI Specialist - Interaction Design** 🔄

### 📚 Continuous Learning
- **Stanford HCI Course** - Advanced Interaction Design
- **MIT xPRO** - AI for Design and Innovation
- **IDEO Design Thinking** - Human-Centered Design
- **WebGL University** - 3D Web Development

## 🌟 Community Contributions

### 📝 Articles & Tutorials
- 📖 ["The Future of UX: AI-Driven Design in 2025"](link) - UX Collective
- 🎨 ["Advanced Framer Motion Patterns"](link) - Dev.to
- 🔬 ["Sustainable UX: Designing for Carbon Neutrality"](link) - Medium

### 🎤 Speaking & Workshops
- **🇹🇭 Thailand UX Conference 2024** - "AI in Design Research"
- **🌐 Figma Config Community** - "Advanced Prototyping Techniques"
- **🎓 University Guest Lectures** - "Career Path: UX to Tech Immigration"

## 📈 Open Source Contributions

### 🛠️ My Libraries
- **[react-gesture-handler-pro](link)** - Advanced touch interactions
- **[figma-to-framer-bridge](link)** - Seamless design handoff
- **[accessibility-audit-toolkit](link)** - Automated a11y testing

### 🤝 Contributions
- **Framer Motion** - Animation performance optimizations
- **React** - Accessibility improvements
- **Three.js** - UX-focused documentation

## 🎯 2025 Goals

### 🚀 Professional Development
- [ ] **Canada Express Entry** - Score 480+ CRS points
- [ ] **FAANG UX Role** - Senior Product Designer position
- [ ] **Climate Tech Impact** - Lead sustainable design initiatives
- [ ] **AI Design Tools** - Develop next-gen design automation

### 🌱 Skill Evolution
- [ ] **WebXR/VR Design** - Spatial computing interfaces
- [ ] **Voice UI Mastery** - Conversational design patterns
- [ ] **Design System AI** - Automated component generation
- [ ] **Global Accessibility** - Universal design principles

<div align="center">

## 🤝 Let's Create the Future Together

[![Portfolio](https://img.shields.io/badge/-🌐%20Portfolio-4285F4?style=for-the-badge&logo=google-chrome&logoColor=white)](link)
[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](link)
[![Dribbble](https://img.shields.io/badge/-Dribbble-EA4C89?style=for-the-badge&logo=dribbble&logoColor=white)](link)
[![Behance](https://img.shields.io/badge/-Behance-1769FF?style=for-the-badge&logo=behance&logoColor=white)](link)

### ⚡ "Design is not just what it looks like and feels like. Design is how it works." - Steve Jobs

*Building bridges between human needs and technological possibilities, one pixel at a time* ✨

</div>


